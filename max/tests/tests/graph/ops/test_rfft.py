...
def expected_output_shape(shape: Shape, n: int | None, axis: int) -> Shape:
    """Compute expected rfft output shape.

    The output has the FFT axis replaced by n // 2 + 1 and a trailing
    dimension of 2 inserted after it for the interleaved complex values.
    """
    axis = axis % len(shape)
    if n is None:
        n = int(shape[axis])
    result = list(shape)
    result[axis] = Dim(n // 2 + 1)
    result.insert(axis + 1, Dim(2))
    return Shape(result)
...
def test_invalid_normalization() -> None:
    input_type = TensorType(DType.float32, (1, 2, 3), DeviceRef.GPU())
    with Graph("rfft", input_types=[input_type]) as graph:
...
def test_invalid_dim() -> None:
    ...
    with Graph("rfft", input_types=[input_type]) as graph:
        with pytest.raises(ValueError, match="Axis dimension must be static"):
            ops.rfft(graph.inputs[0].tensor, n=1, axis=0)
...
def test_invalid_device() -> None:
    ...
    with Graph("rfft", input_types=[input_type]) as graph:
        with pytest.raises(
            ValueError, match=r"RFFT is currently only supported on GPU."
        ):
            ops.rfft(graph.inputs[0].tensor, n=1, axis=0)
