...
mel spectrogram input -> encoder -> CTC logits (zero-padded to the
global max encoder-frame count across all buckets).

On-device argmax was attempted in an earlier version of Step 7 but
caused a 20-minute compile + CUDA OOM on L4 when combined with
per-bucket encoder graphs. Moving argmax on-device is tracked as a
post-bucketing follow-up — the diagnosis there is incomplete and it
needs its own dedicated investigation separate from the bucket refactor.
"""
...
def build_graph(
    ...
    Returns:
        ...
        ``[1, pad_to_encoder_frames, vocab_size]`` (zero-padded along T).
    """
    ...
    with Graph("parakeet_ctc", input_types=[input_type]) as graph:
        ...
        # graph returns the same output shape. The padded region holds
        # blank-biased logits and is sliced off host-side before the
        # greedy decode. Uses ``ops.pad`` so the compiler emits a
        # constant-pad kernel rather than materializing a zero tensor
        # + concat.
