...
mel spectrogram input -> encoder -> on-device argmax -> int32 predicted ids.
...
def build_graph(
    ...
    Returns:
        ...
        ``[1, pad_to_encoder_frames]`` int32 predicted token ids (post
        on-device argmax over the vocab dimension).
    """
    ...
    with Graph("parakeet_ctc", input_types=[input_type]) as graph:
        ...
        # graph returns the same output shape. The pad value is 0 by
        # default; since the host-side code slices to the bucket's true
