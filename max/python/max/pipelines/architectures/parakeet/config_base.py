...
# ===----------------------------------------------------------------------=== #
"""Shared configuration base for Parakeet ASR models.

Both the CTC and TDT architectures use the same FastConformer encoder,
so they share encoder configuration properties. This base class provides
those shared properties; subclasses add model-specific settings.
"""
...
class ParakeetConfigBase(ArchConfig):
    """Shared encoder configuration for Parakeet ASR models.

    Subclasses (``ParakeetModelConfig`` for CTC, ``TDTModelConfig`` for TDT)
    override ``attention_bias``, ``convolution_bias``, and ``blank_id``
    defaults and add model-specific properties.
    """
    ...
    # --- Encoder properties (shared by CTC and TDT) ---
    ...
    def blank_id(self) -> int:
        """Blank token ID. Override in subclasses if different."""
        return self.vocab_size - 1

    @property
    def attention_bias(self) -> bool:
        """Whether attention layers use bias. CTC defaults True, TDT False."""
